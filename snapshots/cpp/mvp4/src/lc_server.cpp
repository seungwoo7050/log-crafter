 ...
 * Track: C++
 * MVP: mvp4
 * Change: Integrate persistence initialization, replay, and statistics into the threaded C++ server runtime.
...
int Server::init(const ServerConfig &config) {
    ...
    std::cerr << "[lc][info] MVP4 C++ server initialized (log=" << config_.log_port
              << ", query=" << config_.query_port
              << ", workers=" << config_.worker_threads
              << ", persistence="
              << (persistence_enabled_ ? config_.persistence_directory : "disabled") << ")" << std::endl;
    return 0;
}
...
void Server::handle_log_client(int client_fd) {
    ...
    const char welcome[] = "LogCrafter C++ MVP4: send newline-terminated log lines.\n";
...
void Server::handle_query_client(int client_fd) {
    ...
    const char banner[] =
        "LogCrafter C++ MVP4 query service.\n"
