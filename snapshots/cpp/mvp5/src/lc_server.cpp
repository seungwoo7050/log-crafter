 ...
 * Track: C++
 * MVP: mvp5
 * Change: Integrate the IRC subsystem with log storage, persistence replay, and runtime statistics.
...
int Server::init(const ServerConfig &config) {
    ...
    std::cerr << "[lc][info] MVP5 C++ server initialized (log=" << config_.log_port
              << ", query=" << config_.query_port
              << ", workers=" << config_.worker_threads
              << ", persistence="
              << (persistence_enabled_ ? config_.persistence_directory : "disabled")
              << ", irc=" << (irc_enabled_ ? std::to_string(config_.irc_port) : std::string("disabled"))
              << ")" << std::endl;
    return 0;
}
...
void Server::handle_log_client(int client_fd) {
    ...
    const char welcome[] =
        "LogCrafter C++ MVP5: send newline-terminated log lines. IRC broadcasting is enabled when configured.\n";
...
void Server::handle_query_client(int client_fd) {
    ...
    const char banner[] =
        "LogCrafter C++ MVP5 query service.\n"
