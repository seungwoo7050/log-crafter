 ...
 * Track: C
 * MVP: mvp3
 * Change: Integrate the enhanced query parser and formatted responses over the thread-pooled server runtime.
...
int lc_server_init(LCServer *server, const LCServerConfig *config) {
    ...
    fprintf(stderr,
            "[lc][info] MVP3 server initialized (log=%d, query=%d, workers=%d, buffer=%zu)\n",
...
void lc_server_shutdown(LCServer *server) {
    ...
    fprintf(stderr, "[lc][info] MVP3 server shutdown complete\n");
}
...
static void lc_log_client_session(LCServer *server, int client_fd) {
    ...
    const char welcome[] = "LogCrafter MVP3: send newline-terminated log lines.\n";
