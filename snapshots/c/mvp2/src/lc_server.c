 ...
 * Track: C
 * MVP: mvp2
 * Change: Replace the single-threaded loop with a thread-pooled server and shared log buffer utilities.
...
static void lc_send_query_keyword(LCServer *server, int client_fd, const char *keyword);
...
int lc_server_init(LCServer *server, const LCServerConfig *config) {
    ...
    fprintf(stderr,
            "[lc][info] MVP2 server initialized (log=%d, query=%d, workers=%d, buffer=%zu)\n",
...
void lc_server_shutdown(LCServer *server) {
    ...
    fprintf(stderr, "[lc][info] MVP2 server shutdown complete\n");
}
...
static void lc_log_client_session(LCServer *server, int client_fd) {
    ...
    const char welcome[] = "LogCrafter MVP2: send newline-terminated log lines.\n";
...
static void lc_send_stats(LCServer *server, int client_fd) {
    ...
    int written = snprintf(response, sizeof(response),
                           ...
                           stats.total_logs, stats.dropped_logs, stats.current_size, clients);
    if (written > 0) {
        lc_send_all(client_fd, response, (size_t)written);
    }
}
...
static void lc_send_query_keyword(LCServer *server, int client_fd, const char *keyword) {
    char **results = NULL;
    size_t count = 0;
    if (lc_log_buffer_copy_keyword(&server->log_buffer, keyword, &results, &count) != 0) {
        const char error[] = "ERROR: Internal query failure.\n";
        lc_send_all(client_fd, error, sizeof(error) - 1);
        return;
    }
    ...
    for (size_t i = 0; i < count; ++i) {
        lc_send_all(client_fd, results[i], strlen(results[i]));
...
static void lc_store_log(LCServer *server, const char *message) {
    if (lc_log_buffer_push(&server->log_buffer, message) != 0) {
