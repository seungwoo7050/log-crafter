 ...
 * Track: C
 * MVP: mvp5
 * Change: Enforce bounded query arguments, limit keyword fan-out, and guard regex size for the security hardening stage.
...
#define LC_QUERY_MAX_ARGUMENT_LENGTH 1024
#define LC_QUERY_MAX_KEYWORDS 16
#define LC_QUERY_MAX_KEYWORD_LENGTH 256
#define LC_QUERY_MAX_REGEX_LENGTH 256
...
static char *lc_query_trim_inline(char *value) {
    if (value == NULL) {
        return NULL;
    }
...
static int lc_query_add_keyword(LCQueryRequest *request, const char *value) {
    ...
    if (request->keyword_count >= LC_QUERY_MAX_KEYWORDS) {
        errno = E2BIG;
        return -1;
    }
...
static int lc_query_parse_keywords(LCQueryRequest *request, const char *value) {
    ...
    while (token != NULL) {
        char *trimmed = lc_query_trim_inline(token);
        if (trimmed == NULL || trimmed[0] == '\0') {
            free(copy);
            errno = EINVAL;
            return -1;
        }
        if (strlen(trimmed) > LC_QUERY_MAX_KEYWORD_LENGTH) {
            free(copy);
            errno = E2BIG;
            return -1;
        }
        if (lc_query_add_keyword(request, trimmed) != 0) {
            free(copy);
            return -1;
        }
...
static int lc_query_compile_regex(LCQueryRequest *request, const char *pattern,
    ...
    if (length > LC_QUERY_MAX_REGEX_LENGTH) {
        errno = E2BIG;
        lc_query_set_error(error_message, error_capacity, "ERROR: Regex parameter too long.");
        return -1;
    }
...
int lc_query_parse_arguments(const char *arguments, LCQueryRequest *request,
    ...
    if (argument_length > LC_QUERY_MAX_ARGUMENT_LENGTH) {
        lc_query_set_error(error_message, error_capacity, "ERROR: Query parameters too long.");
        errno = E2BIG;
        return -1;
    }
    ...
    while (token != NULL) {
        if (strncmp(token, "keyword=", 8) == 0) {
            ...
            char *value = lc_query_trim_inline(token + 8);
            if (value == NULL || value[0] == '\0') {
                lc_query_set_error(error_message, error_capacity,
            ...
            if (strlen(value) > LC_QUERY_MAX_KEYWORD_LENGTH) {
                lc_query_set_error(error_message, error_capacity,
                                   "ERROR: Keyword parameter too long.");
                errno = E2BIG;
                goto error;
            }
            ...
            char *value = lc_query_trim_inline(token + 9);
            if (value == NULL || value[0] == '\0') {
                lc_query_set_error(error_message, error_capacity,
            ...
            char *pattern = lc_query_trim_inline(token + 6);
            if (lc_query_compile_regex(request, pattern, error_message, error_capacity) != 0) {
                goto error;
            }
        } else if (strncmp(token, "time_from=", 10) == 0) {
            char *value = lc_query_trim_inline(token + 10);
            if (value == NULL || value[0] == '\0' ||
                lc_query_parse_time(value, &request->has_time_from, &request->time_from) != 0) {
                lc_query_set_error(error_message, error_capacity,
            ...
            char *value = lc_query_trim_inline(token + 8);
            if (value == NULL || value[0] == '\0' ||
                lc_query_parse_time(value, &request->has_time_to, &request->time_to) != 0) {
                lc_query_set_error(error_message, error_capacity,
