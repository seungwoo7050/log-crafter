 ...
 * Track: C
 * MVP: mvp5
 * Change: Enforce connection limits, guarded metrics, and sanitized log ingestion for the security hardening stage.
...
static void lc_scrub_log_line(char *buffer);
...
static void lc_metrics_snapshot(LCServer *server, int *active, int *pending,
                               unsigned long *rejected);
static int lc_metrics_try_reserve_log_client(LCServer *server);
static void lc_metrics_cancel_log_reservation(LCServer *server);
static int lc_metrics_try_reserve_query_client(LCServer *server);
static void lc_metrics_cancel_query_reservation(LCServer *server);
static void lc_metrics_register_rejection(LCServer *server);
static void lc_send_capacity_notice(int client_fd);
...
LCServerConfig lc_server_config_default(void) {
    ...
    config.max_clients = LC_SERVER_DEFAULT_MAX_CLIENTS;
    return config;
}
...
int lc_server_init(LCServer *server, const LCServerConfig *config) {
    ...
    if (server->config.max_clients <= 0) {
        server->config.max_clients = LC_SERVER_DEFAULT_MAX_CLIENTS;
    }
    ...
    const char *persistence_state = server->config.persistence_enabled
                                        ...
                                        : "disabled";
    fprintf(stderr,
            "[lc][info] MVP5 server initialized (log=%d, query=%d, workers=%d, buffer=%zu, max_clients=%d, persistence=%s)\n",
            server->config.log_port, server->config.query_port,
            server->config.worker_threads, server->config.buffer_capacity,
            server->config.max_clients, persistence_state);
    return 0;
}
...
void lc_server_shutdown(LCServer *server) {
    ...
    fprintf(stderr, "[lc][info] MVP5 server shutdown complete\n");
}
...
int lc_server_run(LCServer *server) {
    ...
    while (server->running) {
        ...
        if (FD_ISSET(server->log_listener_fd, &readfds)) {
            ...
            if (client_fd >= 0) {
                if (!lc_metrics_try_reserve_log_client(server)) {
                    lc_send_capacity_notice(client_fd);
                    close(client_fd);
                } else {
                    LCServerClientJob *job = malloc(sizeof(*job));
                    if (job == NULL) {
                        lc_metrics_cancel_log_reservation(server);
                        lc_metrics_register_rejection(server);
                        lc_send_capacity_notice(client_fd);
                        close(client_fd);
                    } else {
                        job->server = server;
        ...
        if (FD_ISSET(server->query_listener_fd, &readfds)) {
            ...
            if (client_fd >= 0) {
                if (!lc_metrics_try_reserve_query_client(server)) {
                    lc_send_capacity_notice(client_fd);
                    close(client_fd);
                } else {
                    LCServerClientJob *job = malloc(sizeof(*job));
                    if (job == NULL) {
                        lc_metrics_cancel_query_reservation(server);
                        lc_metrics_register_rejection(server);
                        lc_send_capacity_notice(client_fd);
                        close(client_fd);
                    } else {
...
static void lc_metrics_log_client_enter(LCServer *server) {
    ...
    if (server->pending_log_clients > 0) {
        server->pending_log_clients--;
    }
...
static void lc_metrics_query_client_enter(LCServer *server) {
    pthread_mutex_lock(&server->metrics_lock);
    if (server->pending_query_clients > 0) {
        server->pending_query_clients--;
    }
...
static void lc_metrics_snapshot(LCServer *server, int *active, int *pending,
    ...
    pthread_mutex_lock(&server->metrics_lock);
    if (active != NULL) {
        *active = server->active_log_clients + server->active_query_clients;
    }
    if (pending != NULL) {
        *pending = server->pending_log_clients + server->pending_query_clients;
    }
    if (rejected != NULL) {
        *rejected = server->rejected_clients;
    }
    pthread_mutex_unlock(&server->metrics_lock);
}
...
static int lc_metrics_total_clients(LCServer *server) {
    int active = 0;
    int pending = 0;
    lc_metrics_snapshot(server, &active, &pending, NULL);
...
static int lc_metrics_try_reserve_log_client(LCServer *server) {
    ...
    int total = server->active_log_clients + server->active_query_clients +
                server->pending_log_clients + server->pending_query_clients;
    if (total < server->config.max_clients) {
        server->pending_log_clients++;
        allowed = 1;
    } else {
...
static void lc_log_client_session(LCServer *server, int client_fd) {
    ...
    const char welcome[] = "LogCrafter MVP5: send newline-terminated log lines.\n";
...
static void lc_send_help(int client_fd) {
    const char help[] =
        ...
        "STATS - totals, persistence stats, and connection capacity\n"
...
static void lc_send_stats(LCServer *server, int client_fd) {
    ...
    unsigned long rejected_clients = 0;
    lc_metrics_snapshot(server, &active_clients, &pending_clients, &rejected_clients);
    ...
    int written = snprintf(response, sizeof(response),
                           "STATS: Total=%lu, Dropped=%lu, Persisted=%lu, Failed=%lu, Current=%zu, "
                           "ClientsActive=%d, ClientsPending=%d, ClientsRejected=%lu, MaxClients=%d\n",
                           stats.total_logs, stats.dropped_logs, persistence_stats.persisted_logs,
                           persistence_stats.failed_logs, stats.current_size, active_clients,
                           pending_clients, rejected_clients, server->config.max_clients);
    if (written > 0) {
        lc_send_all(client_fd, response, (size_t)written);
    }
}
...
static void lc_send_capacity_notice(int client_fd) {
    const char message[] = "ERROR: Server at capacity. Try again later.\n";
    lc_send_all(client_fd, message, sizeof(message) - 1);
}
...
static ssize_t lc_recv_line(int fd, char *buffer, size_t capacity, int *truncated,
    ...
    size_t guard_limit = capacity * 8;
    if (guard_limit < capacity) {
