...
// [SEQUENCE: 695] Add log with IRC notification
void LogBuffer::addLogWithNotification(const LogEntry& entry) {
    // [SEQUENCE: 696] Add to buffer as normal
    {
        std::unique_lock<std::mutex> lock(mutex_);
    ...
    // [SEQUENCE: 697] Notify IRC channels
    std::unique_lock<std::mutex> lock(mutex_);
    for (const auto& [pattern, callbacks] : channelCallbacks_) {
        // [SEQUENCE: 698] Simple pattern matching for now
        bool matches = false;
        
        if (pattern == "*") {
            matches = true;
        } else if (pattern.find("level:") == 0) {
            std::string level = pattern.substr(6);
            matches = (entry.level == level);
        } else if (pattern.find("keyword:") == 0) {
            std::string keyword = pattern.substr(8);
            matches = (entry.message.find(keyword) != std::string::npos);
        }
...
// [SEQUENCE: 699] Register channel callback
...
// [SEQUENCE: 700] Unregister channel callback
